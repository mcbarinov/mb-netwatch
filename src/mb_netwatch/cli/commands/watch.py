...
def _format_vpn(vpn: ProbeVpn | None) -> str:
...
def _format_ip(ip_probe: ProbeIp | None) -> str:
    """Format cached IP probe for display."""
    if ip_probe is None:
        return "IP:?"
    if ip_probe.ip is None:
        return "IP:?"
    if ip_probe.country_code:
        return f"IP:{ip_probe.ip}({ip_probe.country_code})"
    return f"IP:{ip_probe.ip}"


def _format_row(row: ProbeLatency, vpn: ProbeVpn | None, ip_probe: ProbeIp | None) -> str:
    """Format a single latency row with cached VPN and IP status for terminal display."""
    ts_str = datetime.fromtimestamp(row.created_at, tz=UTC).astimezone().strftime("%H:%M:%S")
    latency_str = "down" if row.latency_ms is None else f"{row.latency_ms:4.0f}ms"
    vpn_str = _format_vpn(vpn)
    ip_str = _format_ip(ip_probe)
    return f"{ts_str}  {latency_str:>6}  {vpn_str}  {ip_str}"


def _make_watch_row(row: ProbeLatency, vpn: ProbeVpn | None, ip_probe: ProbeIp | None) -> WatchRow:
    """Build a WatchRow from the latest latency, VPN, and IP data."""
    ts_str = datetime.fromtimestamp(row.created_at, tz=UTC).astimezone().isoformat()
    return WatchRow(
        ts=ts_str,
        ...
        ip=ip_probe.ip if ip_probe else None,
        country_code=ip_probe.country_code if ip_probe else None,
    )
...
def _poll_loop(app: CoreContext[Core, Output]) -> None:
    ...
    while True:
        ...
        for row in rows:
            ...
            latency_cursor_ts = row.created_at
