...
HISTORY_LIMIT = 200  # Number of most recent probe_ip rows to display


class IpHistoryScreen(Screen[None]):
    """Scrollable table of recent public IP changes, newest first."""
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Static(f"IP history — last {HISTORY_LIMIT} address changes", id="title")
        yield DataTable()
    ...
    def on_mount(self) -> None:
        ...
        table.add_columns("Time", "IP", "Country", "Last seen")
    ...
    def _reload(self) -> None:
        ...
        table = self.query_one(DataTable)
        table.clear()
        for row in self._core.db.fetch_recent_probe_ip(HISTORY_LIMIT):
            ...
            table.add_row(ts, row.ip or "-", row.country_code or "-", last_seen)
