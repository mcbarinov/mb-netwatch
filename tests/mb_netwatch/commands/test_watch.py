...
class TestFormatVpn:
    ...
    def test_inactive(self):
        ...
        vpn = ProbeVpn(created_at=0.0, updated_at=0.0, is_active=False, tunnel_mode="full", provider=None)
        assert _format_vpn(vpn) == "VPN:off"

    def test_active_no_provider(self):
        """Active VPN without provider shows tunnel mode."""
        vpn = ProbeVpn(created_at=0.0, updated_at=0.0, is_active=True, tunnel_mode="full", provider=None)
    ...
    def test_active_with_provider(self):
        ...
        vpn = ProbeVpn(created_at=0.0, updated_at=0.0, is_active=True, tunnel_mode="split", provider="NordVPN")
...
class TestFormatIp:
    ...
    def test_null_ip(self):
        ...
        ip = ProbeIp(created_at=0.0, updated_at=0.0, ip=None, country_code=None)
        assert _format_ip(ip) == "IP:?"

    def test_ip_no_country(self):
        """IP without country shows just the address."""
        ip = ProbeIp(created_at=0.0, updated_at=0.0, ip="1.2.3.4", country_code=None)
        assert _format_ip(ip) == "IP:1.2.3.4"

    def test_ip_with_country(self):
        """IP with country shows address and country in parens."""
        ip = ProbeIp(created_at=0.0, updated_at=0.0, ip="1.2.3.4", country_code="US")
...
class TestFormatRow:
    ...
    def test_normal_latency(self):
        ...
        ts = time.time()
        row = ProbeLatency(created_at=ts, latency_ms=42.0, winner_endpoint="https://example.com")
    ...
    def test_down(self):
        ...
        ts = time.time()
        row = ProbeLatency(created_at=ts, latency_ms=None, winner_endpoint=None)
        result = _format_row(row, None, None)
    ...
    def test_combines_all(self):
        ...
        ts = time.time()
        row = ProbeLatency(created_at=ts, latency_ms=100.0, winner_endpoint="x")
        vpn = ProbeVpn(created_at=0.0, updated_at=0.0, is_active=True, tunnel_mode="full", provider="WG")
        ip = ProbeIp(created_at=0.0, updated_at=0.0, ip="5.6.7.8", country_code="DE")
