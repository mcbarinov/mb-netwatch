...
class TestVpnChecks:
    """Insert, fetch, and purge VPN check rows."""

    def test_insert_and_fetch_latest(self, db):
        """Insert a row and read it back via fetch_latest."""
        ts = datetime.now(tz=UTC)
        db.insert_vpn_check(ts, True, "full", "WireGuard")
    ...
    def test_is_active_bool_conversion(self, db):
        ...
        ts = datetime.now(tz=UTC)
        db.insert_vpn_check(ts - timedelta(seconds=1), True, "full", None)
        db.insert_vpn_check(ts, False, "split", None)

        active_row = db.fetch_latest_vpn_check()
        assert active_row is not None
        assert active_row.is_active is False
        assert isinstance(active_row.is_active, bool)
    ...
    def test_fetch_since(self, db):
        """fetch_since returns only newer rows."""
        now = datetime.now(tz=UTC)
        t1 = now - timedelta(seconds=20)
        t2 = now - timedelta(seconds=10)
        db.insert_vpn_check(t1, True, "full", None)
        db.insert_vpn_check(t2, False, "split", "NordVPN")
    ...
    def test_purge_old(self, db):
        """Purge deletes old rows, keeps recent."""
        now = datetime.now(tz=UTC)
        old = now - timedelta(days=60)
        db.insert_vpn_check(old, True, "full", None)
        db.insert_vpn_check(now, False, "split", None)
...
class TestIpChecks:
    """Insert, fetch, and purge IP check rows."""

    def test_insert_and_fetch_latest(self, db):
        """Insert a row and read it back via fetch_latest."""
        ts = datetime.now(tz=UTC)
        db.insert_ip_check(ts, "1.2.3.4", "US")
    ...
    def test_null_ip_and_country(self, db):
        """None ip and country_code stored and retrieved correctly."""
        ts = datetime.now(tz=UTC)
        db.insert_ip_check(ts, None, None)
        row = db.fetch_latest_ip_check()
        assert row is not None
    ...
    def test_fetch_since(self, db):
        ...
        t1 = now - timedelta(seconds=20)
        t2 = now - timedelta(seconds=10)
        db.insert_ip_check(t1, "1.1.1.1", "US")
        db.insert_ip_check(t2, "2.2.2.2", "DE")
    ...
    def test_purge_old(self, db):
        ...
        now = datetime.now(tz=UTC)
        old = now - timedelta(days=60)
        db.insert_ip_check(old, "1.1.1.1", "US")
        db.insert_ip_check(now, "2.2.2.2", "DE")
