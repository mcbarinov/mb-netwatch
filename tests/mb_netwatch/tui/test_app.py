...
class TestBuildSparkline:
    ...
    def test_renders_chars(self):
        ...
        history = [ProbeLatency(created_at=float(i), latency_ms=float(i * 100), endpoint=None) for i in range(1, 5)]
    ...
    def test_down_shows_cross(self):
        ...
        history = [ProbeLatency(created_at=1.0, latency_ms=None, endpoint=None)]
        result = build_sparkline(history, 300, 800)
